...
//#region node_modules/hyphen/hyphen.js
var require_hyphen = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	*/
	(function(root, factory) {
		...
		else root.createHyphenator = factory();
	})(exports, function() {
		...
		function createHTMLVerifier() {
			...
			return function(accumulate, chars) {
				...
				} else if (chars[0] === "<" && (!isNotLetter(chars[1]) || chars[1] === "/")) skip = true;
				return skip;
			};
		}
		function createHyphenCharVerifier(hyphenChar) {
			...
			return function(accumulate, chars) {
				...
				} else if (!isNotLetter(chars[0]) && chars[1] === hyphenChar) skip = true;
				return skip;
			};
		}
		function createHyphenationVerifier(verifiers, minWordLength) {
			return function() {
				...
				return function(char1, char2) {
					...
					if (!skip) {
						if (isNotLetter(char1) && !isNotLetter(char2)) accumulate();
						if (!isNotLetter(char1) && isNotLetter(char2)) {
							if (accum.length >= minWordLength) return resolveWith([accum0, accum]);
							else accumulate();
						}
					}
					if (char2 === "") {
						if (accum.length < minWordLength || skip) accumulate();
		...
		function insertChar(text, hyphenChar, markers) {
			...
			if (markers.length > 1) for (var i = 0, j = 1; j < markers.length; i++, j++) resultText.push(text.slice(markers[i], markers[j]));
		...
		function markersFromExceptionsDefinition(exceptionsList) {
			return exceptionsList.reduce(function(markersDict, definition) {
				var i = 0, markers = [];
				while ((i = definition.indexOf("-", i + 1)) > -1) markers.push(i);
		...
		function hyphenateWord(text, loweredText, levelsTable, patternTrie) {
			...
			while (wordSlice = nextSlice()) {
				patternEntityIndex++;
				if (isFirstCharacter()) patternEntityIndex--;
				triePtr = patternTrie;
				nextLetter = createCharIterator(wordSlice);
				while (letter = nextLetter()) {
					if ((trieNode = triePtr[letter]) === void 0) break;
					triePtr = {};
					...
					switch (Object.prototype.toString.call(trieNode)) {
						...
						case "[object Object]":
							triePtr = trieNode;
							break;
						case "[object Number]": patternLevelsIndex = trieNode;
					}
					if (patternLevelsIndex < 0) continue;
		...
		function start(text, levelsTable, patterns, cache, markersDict, hyphenChar, skipHTML, minWordLength, isAsync) {
			...
			var newText = "", fragments, readText = createTextReader(createHyphenationVerifier((skipHTML ? [createHTMLVerifier()] : []).concat(createHyphenCharVerifier(hyphenChar)), minWordLength)), resolveNewText = function() {};
			function nextTick() {
		...
		var _global = typeof global === "object" ? global : typeof window === "object" ? window : typeof self === "object" ? self : {};
		...
		function keyOrDefault(object, key, defaultValue, test) {
			if (key in object && (test ? test(object[key]) : true)) return object[key];
			return defaultValue;
		}
		function exceptionsFromMarkers(markers, hyphenChar) {
			var exceptions = {};
			for (var word in markers) exceptions["~" + word] = insertChar(word, hyphenChar, markers[word]);
			return exceptions;
		}
...
//#endregion
export default require_hyphen();
export { require_hyphen };
